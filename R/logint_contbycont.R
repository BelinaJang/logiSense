...
logint_contbycont <- function(formula, variable1, variable2, data, sigfig=4) {
  ...
  sentences <- c(
    ...
    paste0("The 95% CI: e^((", signif(var1_effect, sigfig), " + (", signif(interaction_effect, sigfig), ")*(value)) \u00b1 (1.96)*sqrt(", signif(vcov_m[continuous_var1, continuous_var1],sigfig), ") + ((value)^2)*(", signif(vcov_m[interaction_effect_name, interaction_effect_name],sigfig), ") + 2*(value)*(", signif(vcov_m[continuous_var1, interaction_effect_name],sigfig), "))). \n"
    ),
    ...
    paste0("The 95% CI: e^((", signif(var2_effect, sigfig), " + (", signif(interaction_effect, sigfig), ")*(value)) \u00b1 (1.96)*sqrt(", signif(vcov_m[continuous_var2, continuous_var2],sigfig), ") + ((value)^2)*(", signif(vcov_m[interaction_effect_name, interaction_effect_name],sigfig), ") + 2*(value)*(", signif(vcov_m[continuous_var2, interaction_effect_name],sigfig), "))). \n"
    )
  )
