...
logint_con <- function(formula, variable1, variable2, data, sigfig=4) {
...
will call below function when both variables are continuous (when using multiple r files)
```{r}
logint_con(formula=stroke ~ age*avg_glucose_level, variable1 = "age", variable2 = "avg_glucose_level", data = test_data, sigfig=4)
```
...
logint <- function(formula, variable1, variable2, variable1_type, variable2_type, data, sigfig=4) {
  ...
  if (variable1_type == "continuous" & variable2_type == "continuous"){
    ...
    cat("categorical_var1:", categorical_var1, "categorical_var2:", categorical_var2, "\n")
    ...
    cat("Both variables are of different types. \n")
    
    if (variable1_type == "continuous"){
      continuous_var <- variable1
      cat("Variable 1 is continuous. \n")
      cat("continuous_var:", continuous_var, "\n")
      } else if (variable1_type == "categorical"){
        categorical_var <- variable1
        cat("Variable 1 is categorical. \n")
        cat("categorical_var:", categorical_var, "\n")
      } else {
        warning("variable1_type should be either 'continuous' or 'categorical'.")
      }
    if (variable2_type == "continuous"){
      continuous_var <- variable2
      cat("Variable 2 is continuous. \n")
      cat("continuous_var:", continuous_var, "\n")
    } else if (variable2_type == "categorical"){
        categorical_var <- variable2
        cat("Variable 2 is categorical. \n")
        cat("categorical_var:", categorical_var, "\n")
    } else {
        warning("variable2_type should be either 'continuous' or 'categorical'.")
    }
 }
...
```{r}
logint(formula=stroke ~ age*avg_glucose_level, variable1 = "work_type", 
