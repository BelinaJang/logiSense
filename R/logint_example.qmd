  ...
  for (i in 1:(length(interaction_terms)+1)) {
    ...
    odds_ratio_sentences <- c(
      ...
      paste0(
        ...
        categorical_levels[i], " = ", signif(odds_ratios[i], 4),
        "."
      ), paste0(
          "The 95% CI: e^(", base_effect, " ± (1.96)*(",
          vcov_m[odds_ratios_names[1], odds_ratios_names[1]], " + ",
          vcov_m[odds_ratios_names[i], odds_ratios_names[i]], " + ",
          vcov_m[odds_ratios_names[1], odds_ratios_names[i]], ")) = (", CI[1], ", ", CI[2], ").\n"
      )
      )
  }
  ...
  for (i in seq_along(interaction_terms)) {
    relative_odds_sentences <- c(
      ...
      paste0(
        ...
        vcov_m[categorical_effects_names[i], categorical_effects_names[i]], " + ",
        vcov_m[interaction_terms_names[i], interaction_terms_names[i]], " + 2*(value) ",
        vcov_m[categorical_effects_names[i], interaction_terms_names[i]], "))."
      )
    )
  }
